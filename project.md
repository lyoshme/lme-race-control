...
└── src/
    ...
    │   ├── layout/         # Шапка, подвал и общая разметка
    │   ├── toast/          # Контекст и UI для всплывающих уведомлений
    │   └── ui/             # Базовые UI элементы (кнопки, инпуты, модалки)
    ├── hooks/              # Пользовательские хуки (Auth, Theme, Supabase)
    ├── lib/                # Вспомогательные библиотеки и вычисления
    │   ├── api/            # API-слой для взаимодействия с Supabase
    ...
    └── features/           # Функциональные модули
        ├── auth/           # Компоненты авторизации и OTP
        ├── championship/   # Логика и формы чемпионатов
        ├── drivers/        # Управление пилотами (включая DnD)
        ├── scoring/        # Управление системами начисления очков
        ├── stages/         # Мастер проведения этапа (Wizard) и история
        ├── standings/      # Отображение таблиц зачетов с анимациями
